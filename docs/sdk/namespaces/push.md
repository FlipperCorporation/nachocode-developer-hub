...
:::info 마케팅 푸시 관련
`registerPushToken()` 호출 시 내부적으로 `Nachocode.user.setUserId()`가 호출되어 **로그인 (`guest` -> `user`) 상태로 전환**됩니다.
...
:::info 마케팅 푸시 관련
`deletePushToken()` 호출 시 내부적으로 `Nachocode.user.deleteUserId()`가 호출되어 **비로그인 (`user` -> `guest`) 상태로 전환**됩니다.
