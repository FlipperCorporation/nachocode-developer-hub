...
앱 스토어의 경우 정책상 개발자가 환불이 **불가능**합니다. Nachocode 대시보드에서 iOS의 결제 상태가 **검증 실패**일 경우 아래 [에러 케이스](#인앱-결제-에러-케이스)를 확인하여 조치 해주세요.

검증이 **완료된 건에 대해서만** 웹훅을 전송합니다.
...
해당 식별자는 **환불 시에도 동일**하게 사용됩니다.

### 6. 클라이언트 응답 처리

인앱결제 응답에 따른 클라이언트 처리가 필요합니다. [링크](../sdk/namespaces/iap.md#사용-예제)를 통해 인앱결제 SDK 호출 이후 핸들링 **예시**를 확인할 수 있습니다.
