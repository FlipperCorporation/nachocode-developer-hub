...
**다수의 유저에게 동일**한 메시지를 보낼 수도 있고, **각 유저마다 다른 내용의 푸시 메시지**를 동적으로 생성하여 전송할 수도 있어 다양한 **개인화 마케팅**과 **기능성 알림 시나리오**에 적합합니다. 또한, 추가적인 예약 전송 옵션을 통해 특정 시간에 푸시 알림이 발송되도록 **자동화**할 수 있습니다.

:::info

[`/messages`](../../api/push/personal-push.endpoints#post-v2-messages)로 끝나는 API 엔드포인트의 경우, **각기 다른 유저에게 다른 제목과 메세지를 전송**할 때 사용하고,  
[`/users`](../../api/push/personal-push.endpoints#post-v2-users)로 끝나는 API 엔드포인트의 경우, **다수의 유저에게 동일한 제목과 내용을 전송**할 때 사용합니다.
...
- **예약 기능을 활용한 자동화 푸시**
  ...
  - 푸시 전송 예약을 통해 워킹 타임에 구애받지 않고, 최적의 타이밍에 메시지를 전달하여 운영 효율을 높입니다.
  - 사용자 경험과 전환율을 동시에 개선할 수 있습니다.
...
### 3. 개인화 푸시 전송 ([API](../../api/push/personal-push.endpoints))
...
**동일한 내용**을 **다수의 유저**에게 전송할 시에는 [`/users`](../../api/push/personal-push.endpoints#post-v2-users) API Endpoint를 사용하고,  
**각 유저마다 다른 내용**을 전송할 시에는 [`/messages`](../../api/push/personal-push.endpoints#post-v2-messages) API Endpoint를 사용합니다.
